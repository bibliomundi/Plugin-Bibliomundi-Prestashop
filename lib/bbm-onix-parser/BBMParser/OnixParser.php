				...
				foreach ($xmlProduct->MainSubject as $mainSubject)
				{
					...
					switch (strval($mainSubject->MainSubjectSchemeIdentifier))
					{
						case '10'://Bisac
							...
							try
							{
								$category = new Bisac(strval($mainSubject->SubjectCode), strval($subject->SubjectHeadingText));
							}
							catch(\Exception $e)
							{
								unset($category);
						...
						case '01'://CDD
							...
							try
							{
								$category = new CDD(strval($mainSubject->SubjectCode), strval($subject->SubjectHeadingText));
							}
							catch(\Exception $e)
							{
								unset($category);
